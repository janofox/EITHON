    ...
    def filter(self, event):
        ...
        if self.require_admin and not isinstance(event._chat_peer, types.PeerUser):
            ...
            if not is_creator and not is_admin:
                text = "** 𓆩𓆩︙ اخوي احتاج لصلاحيه الادمن 𓆪𓆪**"
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f" [الرابط]({msglink}) **𓆩𓆩: عذرًا ، لا يمكنني إرسال هذه الرسالة في الدردشات العامة ، فقد تحتوي على بعض البيانات الحساسة 𓆪𓆪**"
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            reply_to=reply_to,
