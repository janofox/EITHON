...
async def on_sql(event):  # sourcery no-metrics
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.sql_type == "media":
            if AFK_.reason:
                message_to_reply = (f"**𓆩𓆩︙ عـذرا انا الان في وضـعـيه عـدم الاتـصـال 𓆪𓆪** .\n**𓆩𓆩︙ وضـع عـدم الاتصــال منذ 🕐 :** `{endtime}` 𓆪𓆪")
            else:
                message_to_reply = f"**𓆩𓆩︙ عذرا انا الان في وضـعـيه عدم الاتـصال 𓆪𓆪** .\n**𓆩𓆩︙ وضـع عـدم الاتصــال مـنذ 🕐 :** `{endtime}` 𓆪𓆪"
            if event.chat_id:
                msg = await event.reply(message_to_reply, file=AFK_.media_sql.media)
        elif AFK_.sql_type == "text":
            if AFK_.msg_link and AFK_.reason:
                message_to_reply = (f"**𓆩𓆩︙ عـذرا انا الـان في وضـعـيه عدم الاتـصال 𓆪𓆪** .\n**𓆩𓆩︙ وضـع عـدم الاتـصال منذ 🕐 :** `{endtime}` 𓆪𓆪")
            elif AFK_.reason:
                message_to_reply = (f"**𓆩𓆩︙ عـذرا انا الـان في وضـعيه عدم الاتـصال 𓆪𓆪** .\n**𓆩𓆩︙ وضـع عدم الاتـصـال مـنذ 🕐 :** `{endtime}` 𓆪𓆪")
            else:
                message_to_reply = f"**𓆩𓆩︙ عـذرا انا الان فـي وضـعيه عدم الاتـصال 𓆪𓆪** .\n\n**𓆩𓆩︙ وضـع عـدم الاتـصال منذ 🕐 :** `{endtime}` 𓆪𓆪"
            if event.chat_id:
        ...
        resalt = f"<b>⌔︙ الـمجمـوعات 𖠄 : </b><code>{hmm.title}</code>"
        if full is not None:
            resalt += f"\n<b>⌔︙ مـن 𖠒 : </b> 👤{_format.htmlmentionuser(full.first_name , full.id)}"
